...
export function AegisNode({ data, selected }: { data: AegisNodeData; selected: boolean }) {
  ...
  const showPressure = risk > 0.6;
  const pressureColor = risk > 0.85 ? "rgba(192, 132, 123, 0.15)" : "rgba(217, 119, 6, 0.1)"; // Rose or Amber
  
  return (
    <div className="relative">
      {/* Architectural Pressure Halo */}
      {showPressure && !selected && (
        <div 
          className="absolute -inset-6 rounded-2xl pointer-events-none z-0 blur-xl transition-opacity duration-1000"
          style={{ background: `radial-gradient(circle, ${pressureColor} 0%, transparent 70%)` }}
        />
      )}
      ...
      <motion.div
        ...
        className={`relative z-10 px-5 py-4 min-w-[240px] transition-all duration-500 bg-stone-900/60 backdrop-blur-xl border border-stone-800/40 ${
          selected 
            ? "border-stone-500 shadow-[0_0_30px_rgba(0,0,0,0.4)]" 
            : data.isBlastRadius
            ? `border-stone-700 bg-stone-800/30 shadow-lg`
            : "shadow-2xl"
        }`}
      >
        {/* Visual Anchor Line */}
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {data.metadata.centrality !== undefined && data.metadata.centrality > 0.1 && (
            <div className="h-1.5 w-1.5 rounded-full bg-stone-500" title="Core Module" />
          )}
        </div>
